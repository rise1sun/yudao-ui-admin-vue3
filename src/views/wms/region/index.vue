<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="区域前缀" prop="prefix">
        <el-input
...
<!--      <el-table-column label="区域id" align="center" prop="id" />-->
      <el-table-column label="区域前缀" align="center" prop="prefix" />
  ...
  <!-- 子表的列表 -->
  <ContentWrap>
    <el-tabs model-value="regionStorage">
      <el-tab-pane label="库位" name="regionStorage">
        <RegionStorageList :region-id="currentRow.id" />
      </el-tab-pane>
    </el-tabs>
  </ContentWrap>
</template>
...
import RegionStorageList from './components/RegionStorageList.vue'
...
const getList = async () => {
  loading.value = true
  try {
    const data = await RegionApi.getRegionPage(queryParams)
